...
// Helper: attach headers + body + token if present
...
export function createClassroom({ name, year, role_id }) {
  return sendRequest('/classrooms/', 'POST', { name, year, role_id });
}
...
export function recordAttendance({ date, status, note, role_id, classroom_id }) {
  return sendRequest('/attendance/', 'POST', { date, status, note, role_id, classroom_id });
}
...
export function createReport({ title, content, role_id, attendance_id }) {
  return sendRequest('/reports/', 'POST', { title, content, role_id, attendance_id });
}
